...
telegram_bot = Updater(token=TELEGRAM_TOKEN, use_context=True).bot
    ...
    for user_id, email in cursor.execute('SELECT user_id, email FROM emails'):
        ...
        for message in reversed(messages):  # Читаем сообщения в порядке, обратном их получению
            ...
            if message_text:
                ...
                telegram_bot.send_message(chat_id=user_id, text=telegram_message)

# Команда для добавления адресата
def add_email_start(bot, update):
    update.message.reply_text('Введите адрес электронной почты, от которого вы хотите получать уведомления:')
...
def add_email(bot, update):
    ...
    conn.commit()
    update.message.reply_text(f'Адрес {email} успешно добавлен.')

# Добавляем хэндлер для обработки ответа пользователя
updater.dispatcher.add_handler(MessageHandler(Filters.regex(r'[1]+@[\w.-]+.\w+'), add_email), group=1)
...
def add_email_cancel(bot, update):
    update.message.reply_text("Добавление адресата отменено.", reply_markup=ReplyKeyboardRemove())
...
updater.dispatcher.add_handler(add_email_handler)

# Команда для просмотра списка адресатов
def view_emails(bot, update):
    user_id = update.message.chat_id
    emails = [row[1] for row in cursor.execute('SELECT email FROM emails WHERE user_id = ?', (user_id,))]
    if emails:
...
# Создание обработчика команды и запуск бота
updater = Updater(TELEGRAM_TOKEN)
updater.dispatcher.add_handler(CommandHandler('add_email', add_email, pass_args=True))
updater.dispatcher.add_handler(CommandHandler('view_emails', view_emails))
